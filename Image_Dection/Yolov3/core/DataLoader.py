...
class DataLoader:
    ...
    """
    def __init__(self,
                 data_path = cfg.annotation_path,#txt文件路径
                 input_shape = cfg.input_shape,  #图片大小
                 batch_size =  cfg.batch_size, #batch的大小
                 max_boxes = 20 #最大的boxes的数量
                 ):
    ...
    def read_data_and_split_data(self):
        ...
        np.random.shuffle(files) #将数据进行打乱
    ...
    def parse(self,annotation_line):
        ...
        # print("y_true13.shape:",y_true_13.shape,":",type(y_true_13))
        # print("y_true_26.shape:", y_true_26.shape)
        # print("y_true_52.shape:", y_true_52.shape)
        # -------------------------------需要修改--------------------------------------------
        # y_true_13.set_shape([13, 13, 3, 25])
        # y_true_26.set_shape([26, 26, 3, 25])
        # y_true_52.set_shape([52, 52, 3, 25])
        y_true_13.set_shape([13, 13, 3, 5 + cfg.num_classes])
        y_true_26.set_shape([26, 26, 3, 5 + cfg.num_classes])
        y_true_52.set_shape([52, 52, 3, 5 + cfg.num_classes])
        box_data = y_true_13, y_true_26, y_true_52

        # print("box_data.shape:", box_data)
